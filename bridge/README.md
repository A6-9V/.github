...
2.  Install the required dependencies:
    ...
    ```
3.  Set the environment variables for Security:
    *   **API Key** (Required for authentication):
        ```bash
        export JULES_BRIDGE_API_KEY="your_secure_key"
        ```
    *   **IP Allow-listing** (Optional, recommended for extra security):
        ```bash
        export ALLOWED_IPS="127.0.0.1,your.vps.ip,your.device.ip"
        ```
4.  Run the server:
    ```bash
    python server.py
